...
var TableScanner = exports.TableScanner = function (tableClient, scanRequest) {
  ...
  this.splitIndex = scanRequest.splitIndex;
  this.retryTime = 0;
...
TableScanner.prototype.forEach = function (callback) {
  ...
  self.scanRequest.splitIndex = self.splitIndex;
  self.tableClient.scan(self.scanRequest, function (error, result) {
    if (error) {
      callback(error, null);
    } else {
      if (result.nextStartKey == null && result.nextSplitIndex == -1) {
        self.finish = true;
      } else {
        ...
        if (result.nextSplitIndex > 0) {
          self.splitIndex = result.nextSplitIndex;
        }
      }
